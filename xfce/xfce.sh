...
[ ! "$EUID" -eq 0 ] && echo -e "É necessário rodar o script como root!\n${_g}Use:${_o} ${_w}sudo ./xfce.sh${_o}" && exit 1
...
# keyboard
curl -s -o /etc/X11/xorg.conf.d/10-evdev.conf 'https://raw.githubusercontent.com/leoarch/arch-install/master/keyboard'
curl -s -o /etc/X11/xorg.conf.d/30-touchpad.conf 'https://raw.githubusercontent.com/leoarch/arch-install/master/touchpad'
